<template>
  <div ref="message-holder" class="mini-track messages">
    <div
      ...
      <div class="message"
           :class="item.class">
        <div class="message__row row">
          <div class="message__row-content">{{ item.message }}</div>
          <div v-if="item.closeable"
               class="message__row-button is-clickable icon"
               @click="$message.remove(item)">
            <svg class="close" viewBox="0 0 200 200" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" xml:space="preserve" xmlns:serif="http://www.serif.com/" style="fill-rule:evenodd;clip-rule:evenodd;stroke-linejoin:round;stroke-miterlimit:2;">
